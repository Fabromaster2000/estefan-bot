...
async function handle({ sessionId, phone, text }) {
  ...
  if (session.step === 'COLOR_DETALLE_TIEMPO') {
    ...
    session.step = 'COLOR_DETALLE_COLOR';
    return send(`¡Gracias! 💛 ¿Y cuál es el resultado que estás buscando?\n\nEj: "quiero ser rubia platinada", "mechitas caramelo sobre el castaño", "castaño oscuro con luces"... Cuanto más detalle, mejor se preparan las estilistas ✨`);
  }
  ...
  if (parsed.servicio2 && !session.data.extra) {
    ...
    if (srv2) {
      // Si el extra requiere consulta y el servicio principal NO la requiere,
      // invertir: color debe ser el servicio principal para que pase por el flujo de consulta
      if (srv2.consulta && !session.data.servicio?.consulta) {
        session.data.extra = session.data.servicio; // el corte pasa a ser el extra
        session.data.servicio = srv2;               // el color es el principal
        session.data.servicioConfirmado = false;
        session.data.consultaOk = false;
        console.log(`[orch] servicio2 con consulta → invertido: principal=${srv2.nombre}, extra=${session.data.extra?.nombre}`);
      } else {
        session.data.extra = srv2;
      }
      session.data.upsellOfrecido = true;
      console.log(`[orch] servicio2: ${srv2.nombre}`);
    }
  }
  ...
  // ── Routing ───────────────────────────────────────────────────────────────
  if (intent === 'PRECIO') {
    // Si mencionó servicios específicos → mostrar solo esos precios
    const SERVICIOS_PRECIOS = [
      { re: /corte.*brushing|brushing.*corte/i, txt: '✂️ *Corte + Brushing:* $70.000' },
      { re: /corte/i,       txt: '✂️ *Corte de pelo:* $50.000 (incluye lavado y aireado)' },
      ...
      { re: /color entero|tintura|teñi|tinte/i, txt: '🎨 *Color entero:* desde $90.000' },
      { re: /color/i,   txt: '🎨 *Color entero:* desde $90.000' },
      { re: /peinado.*novia|novia/i, txt: '💐 *Peinado novia:* desde $150.000' },
      { re: /peinado.*fiesta|fiesta|15/i, txt: '💐 *Peinado fiesta \/15:* desde $70.000' },
      { re: /head spa|spa/i, txt: '💆 *Head Spa completo:* $120.000' },
      { re: /ozono/i,       txt: '💆 *Ozono:* $30.000' },
      { re: /ampolla/i,     txt: '✨ *Ampolla:* $30.000' },
    ];
    const mencionados = SERVICIOS_PRECIOS.filter(s => s.re.test(t));
    if (session.step === 'RESERVANDO') session.step = 'RESERVANDO';
    if (mencionados.length > 0) {
      const lista = mencionados.map(s => '  • ' + s.txt).join('\n');
      const intro = parsed.texto && !/\$[0-9]/.test(parsed.texto) ? parsed.texto + '\n\n' : '';
      const cierre = '\n\n_¿Querés ver todos los servicios y precios, o ya tenés todo para reservar? 💛_';
      return send(intro + lista + cierre);
    }
    // No mencionó servicio específico → lista completa
    const intro = parsed.texto && !/\$[0-9]/.test(parsed.texto) ? parsed.texto + '\n\n' : '';
  ...
  if (intent === 'RESERVAR' || session.step === 'RESERVANDO') {
    ...
    if (srvDetectado && srvDetectado.id !== session.data.servicio?.id) {
      // Si ya había un servicio principal sin consulta, lo pasamos a extra
      // El color/químico SIEMPRE debe ser el servicio principal para pasar por el flujo de consulta
      if (srvDetectado.consulta && session.data.servicio && !session.data.servicio.consulta) {
        session.data.extra = session.data.servicio; // corte → extra
        session.data.upsellOfrecido = true;
      }
...
async function avanzarReserva(session, phone, parsed, send, clientCtx) {
  ...
  // También aplica si el extra es un servicio de color (fallback de seguridad)
  const servicioRequiereConsulta = d.servicio?.consulta || d.extra?.consulta;
  if (servicioRequiereConsulta && !d.consultaOk) {
    // Si el extra requiere consulta pero el principal no, promover el extra a principal ahora
